...
with col_main1:
    ...
    st.session_state.irrad_data = st.session_state.irrad_data.assign(
        kWh=st.session_state.irrad_data['shortwave_radiation_sum'] * st.session_state.panels_area * st.session_state.panels_rend / 100
    )
    fig1 = px.line(
    ...
    # pivot_irrad = (irrad_data[(irrad_data['time'] >= ten_years_ago)]
    #                .pivot(index='time', values='shortwave_radiation_sum', columns='true_value')
    #                .rename(columns={1: 'actual', 0: 'predicted'})
    #                .reset_index())

    # st.line_chart(data=pivot_irrad, x='time', y=['actual', 'predicted'])

with col_main2:
    # user inputs
    st.session_state.panels_area = st.number_input('📐 Área total em m² dos painéis solares', step=5, min_value=2, value=18)
    st.session_state.panels_rend = st.slider('💱 Eficiência média dos painéis solares', min_value=0, max_value=100, step=1, format='%d%%',
                     value=15)
    number = st.number_input('💵 Tarifa média por kWh (R$)', min_value=0.05, max_value=2., step=0.001, value=0.656, format='%.3f')
